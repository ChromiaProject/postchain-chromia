        ...
        script {
          postgresContainerIpAddress = sh(
            script: """
              ...
              export POSTGRES_CONTAINER_ID=`docker run -d --name postgres -e POSTGRES_INITDB_ARGS="--lc-collate=C.UTF-8 --lc-ctype=C.UTF-8 --encoding=UTF-8" -e POSTGRES_PASSWORD=postchain -e POSTGRES_USER=postchain -p 5433:5432 postgres`
      ...
      steps {
        withCredentials([
          ...
          sh """
            ...
            mvn $MAVEN_CLI_OPTS --activate-profiles ci,gitlab-registry,distro,nightly -Dlatest.tag=latest clean deploy
          """
        }
      }
    }
