...
class Directory1TransactionSubmitterIT {
    ...
    private fun assertAnchoringInProgress(signers: List<PubKey>, awaitAnchoredHeights: Int = 3) {
        ...
        Awaitility.await().atMost(3, TimeUnit.MINUTES).untilAsserted {
            ...
            txsClient.getTransactions(null, null, null, null, null, null, count = 100L).forEach {
        ...
        Awaitility.await().atMost(3, TimeUnit.MINUTES).untilAsserted {
            ...
            assertThat(txsClient.getTransactions(null, null, null, null, null, null, count = 100L).count { it.status == TransactionStatus.SUCCESS }).isGreaterThanOrEqualTo(1)
        }
    }
}