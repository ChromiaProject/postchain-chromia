        ...
        if (batch) {
            client.transactionBuilder()
                    .proposeProvidersOperation(
                            client.pubkey, provider, providerTier.toTier(), providerTier.isSystem(), enable, message
                    )
            ...
            client.transactionBuilder()
                    ...
                    .apply {
                        if (providerTier.shouldEnable(enable)) proposeProviderStateOperation(client.pubkey, pubkeys.first().data, enable, message)
                        if (providerTier == ProviderType.SYSTEM_PROVIDER) proposeProviderIsSystemOperation(client.pubkey, pubkeys.first().data, true, message)
                    }
                    .postAwaitConfirmation()
                    .printResult(
