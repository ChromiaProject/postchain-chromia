...
tar xf "$PMC"
tar xf "$DEPLOY"
bash "$D"/chromia-deploy compile --source-dir "$D/../directory1/rell" --output-dir "$D/../out" config/c0-deploy.xml
bash "$D"/chromia-deploy compile --source-dir "$D/../app/src" --output-dir "$D/../app-out" "$D/../app/config/deploy.xml"
