...
(ii) Import blocks to the node (see also point 7):
...
7. _Monitoring of syncing and anchoring._ Syncing and anchoring might take a long time for big blockchains. To skip the sync step, one can import blocks on _every_ node in parallel on step 6.ii. Anchoring can be sped up by adjusting `anchoring.max_blocks_per_chain` blockchain configuration parameter of the cluster anchoring chain, which equals 100 by default.  
